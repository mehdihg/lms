...
const generateDarkenColorFrom =(input:string,opacity=0.07)=>
colord(input).darken(opacity).toHex()

const generateForegroundColorFrom =(input:string,opacity=0.8)=>
colord(input).mix(colord(input).isDark()? 'white':'black',opacity).toHex()
